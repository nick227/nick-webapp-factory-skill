...
# Template: TypeScript config placement

Copy from `templates/configs/` into the project at these exact paths:

| Source | Destination | Notes |
|---|---|---|
| `templates/configs/tsconfig.base.json` | `tsconfig.base.json` (repo root) | Extended by all packages; do not edit per-package |
| `templates/configs/tsconfig.server.json` | `apps/server/tsconfig.json` | Extends `../../tsconfig.base.json` |
| `templates/configs/tsconfig.app.json` | `apps/web/tsconfig.json` | Extends `../../tsconfig.base.json`; React/Vite JSX settings |

`packages/api-spec`, `packages/sdk`, and `packages/db` extend `../../tsconfig.base.json` with minimal per-package overrides — no separate template needed; write them inline.
